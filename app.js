...
function waLink(msg){ return BIZ.whatsapp ? `https://wa.me/${BIZ.whatsapp}?text=${encodeURIComponent(msg)}` : '#'; }
function sendOrder(){
  ...
  if(BIZ.whatsapp) window.open(waLink(`Hello ${BIZ.name}! I would like to order:\n${lines}\nTotal: ${money(total)}\nName: ${name}\nPhone: ${phone}${f.notes.value?'\nNotes: '+f.notes.value:''}`),'_blank');
  document.getElementById('order-success').classList.remove('hidden');
  document.getElementById('order-summary').textContent=`Total ${money(total)}${BIZ.whatsapp?' - your order is ready to send on WhatsApp.':' - we will call you to confirm.'}`;
}
function sendReserve(){
  ...
  if(BIZ.whatsapp) window.open(waLink(`Hello ${BIZ.name}! I would like to ${type}:\nDate: ${f.date.value} ${f.time.value}\nGuests: ${f.party_size.value}${room}\nName: ${name}\nPhone: ${phone}`),'_blank');
  document.getElementById('reserve-success').classList.remove('hidden');
  document.getElementById('reserve-summary').textContent=`${f.date.value} at ${f.time.value}${BIZ.whatsapp?' - ready to send on WhatsApp.':' - we will confirm by phone.'}`;
}
